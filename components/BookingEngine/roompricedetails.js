...
function RoomPriceDetails({ setDisplay }) {
    ...
    function Booknow() {
        return (<><div className='w-full h-full mt-5  pt-4 px-3 flex flex-col gap-2 bg-slate-200 rounded-2xl text-slate-600 font-semibold'>
            ...
            <h2 className='text-blue-900 border-t border-black'>Base Amount: ₹ {rate?.final_rate}</h2>
            <h2 className='text-blue-900 border-t border-black '>Tax Amount: ₹ {rate?.tax_amount}</h2>
            <h2 className='text-blue-900 border-t border-black '>Other Fees: ₹ {rate?.otherfees_amount}</h2>
            <h2 className='text-blue-900 border-t border-black '>Total Price Per Day: ₹ {rate?.final_rate + rate?.otherfees_amount + rate?.tax_amount}</h2>
